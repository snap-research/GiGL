...
class NodeAnchorBasedLinkPredictionTaskTest
  ...
  private def getSplits[A](
    samples: Dataset[A],
    splitFn: (A, DatasetSplits.DatasetSplit) => Seq[A],
  )(implicit encoder: Encoder[A],
  ): (Dataset[A], Dataset[A], Dataset[A]) = {
    val trainSplit = samples.flatMap(row => splitFn(row, DatasetSplits.TRAIN))
    val valSplit   = samples.flatMap(row => splitFn(row, DatasetSplits.VAL))
    val testSplit  = samples.flatMap(row => splitFn(row, DatasetSplits.TEST))
