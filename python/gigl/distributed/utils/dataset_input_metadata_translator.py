...
from gigl.distributed.dataset.dist_link_prediction_dataset import DatasetInputMetadata
...
def _build_serialized_tfrecord_entity_info(
    ...
    return SerializedTFRecordInfo(
        ...
        feature_keys=preprocessed_metadata.feature_keys,
...
def convert_pb_to_dataset_input_metadata(
    ...
    """
    Populates a DatasetInputMetadata field from PreprocessedMetadataPbWrapper and GraphMetadataPbWrapper, containing information for loading tensors for all entities and node/edge types.
    Args:
        preprocessed_metadata_pb_wrapper (PreprocessedMetadataPbWrapper): Preprocessed Metadata Pb Wrapper to translate into DatasetInputMetadata
        graph_metadata_pb_wrapper (GraphMetadataPbWrapper): Graph Metadata Pb Wrapper to translate into Dataset Metadata
    Returns:
        DatasetInputMetadata: Dataset Metadata for all entity and node/edge types.
    """
    ...
    if not graph_metadata_pb_wrapper.is_heterogeneous:
        ...
        return DatasetInputMetadata(
            node_entity_info=list(node_entity_info.values())[0],
            edge_entity_info=list(edge_entity_info.values())[0],
            positive_label_entity_info=list(positive_label_entity_info.values())[0],
            negative_label_entity_info=list(negative_label_entity_info.values())[0],
        )
    else:
        return DatasetInputMetadata(
