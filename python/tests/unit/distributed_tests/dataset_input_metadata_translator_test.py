...
from gigl.distributed.utils.dataset_input_metadata_translator import (
    convert_pb_to_dataset_input_metadata,
)
...
class DistRandomPartitionerTestCase(unittest.TestCase):
    ...
    def _assert_data_type_correctness(
        ...
        """
        Checks that each item in the provided dataset input metadata is correctly typed and, if heterogeneous, that edge types and node types are as expected.
        Args:
            entity_info: Optional[
                Union[
                    SerializedTFRecordInfo,
    ...
    def test_translator_correctness(self, _, mocked_dataset_info: MockedDatasetInfo):
        ...
        dataset_input_metadata = convert_pb_to_dataset_input_metadata(
        ...
        self._assert_data_type_correctness(
            dataset_input_metadata.node_entity_info,
            is_heterogeneous=graph_metadata_pb_wrapper.is_heterogeneous,
            expected_entity_types=graph_metadata_pb_wrapper.node_types,
        )

        if isinstance(dataset_input_metadata.node_entity_info, abc.Mapping):
            serialized_node_info_iterable = list(
                dataset_input_metadata.node_entity_info.values()
            )
        else:
            serialized_node_info_iterable = [dataset_input_metadata.node_entity_info]
        ...
        self._assert_data_type_correctness(
            dataset_input_metadata.edge_entity_info,
            is_heterogeneous=graph_metadata_pb_wrapper.is_heterogeneous,
            expected_entity_types=graph_metadata_pb_wrapper.edge_types,
        )

        if isinstance(dataset_input_metadata.edge_entity_info, abc.Mapping):
            serialized_edge_info_iterable = list(
                dataset_input_metadata.edge_entity_info.values()
            )
        else:
            serialized_edge_info_iterable = [dataset_input_metadata.edge_entity_info]
        ...
        if has_positive_labels:
            self._assert_data_type_correctness(
                dataset_input_metadata.positive_label_entity_info,
                is_heterogeneous=graph_metadata_pb_wrapper.is_heterogeneous,
                expected_entity_types=graph_metadata_pb_wrapper.edge_types,
            )
            if isinstance(
                dataset_input_metadata.positive_label_entity_info, abc.Mapping
            ):
                serialized_positive_label_info_iterable = list(
                    dataset_input_metadata.positive_label_entity_info.values()
                )
            else:
            ...
            for edge_type, seralized_positive_label_info in zip(
                ...
                if preprocessed_metadata_pb_wrapper.has_pos_edge_features(
                    ...
                    self.assertEqual(
                        ...
                        preprocessed_metadata_pb_wrapper.condensed_edge_type_to_pos_edge_feature_schema_map[
                            condensed_edge_type
                        ].feature_spec,
                    )
                else:
            ...
            self.assertIsNone(dataset_input_metadata.positive_label_entity_info)
        ...
        if has_negative_labels:
            self._assert_data_type_correctness(
                dataset_input_metadata.negative_label_entity_info,
                is_heterogeneous=graph_metadata_pb_wrapper.is_heterogeneous,
                expected_entity_types=graph_metadata_pb_wrapper.edge_types,
            )
            if isinstance(
                dataset_input_metadata.negative_label_entity_info, abc.Mapping
            ):
                serialized_negative_label_info_iterable = list(
                    dataset_input_metadata.negative_label_entity_info.values()
                )
            else:
            ...
            for edge_type, serialized_negative_label_info in zip(
                ...
                if preprocessed_metadata_pb_wrapper.has_hard_neg_edge_features(
                    ...
                    self.assertEqual(
                        ...
                        preprocessed_metadata_pb_wrapper.condensed_edge_type_to_hard_neg_edge_feature_schema_map[
                            condensed_edge_type
                        ].feature_spec,
                    )
                else:
            ...
            self.assertIsNone(dataset_input_metadata.negative_label_entity_info)
