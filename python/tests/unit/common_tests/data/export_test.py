...
class TestEmbeddingExporter(unittest.TestCase):
    ...
    @patch("gigl.common.data.export.FileLoader")
    def test_load_embedding_to_bigquery(self, mock_file_loader, mock_bigquery_client):
        # Mock inputs
        ...
        mock_file_loader.return_value = mock_loader
        mock_loader.list_children.return_value = [
            GcsUri("gs://test-bucket/test-path/file1.avro")
        ]
        ...
        mock_loader.list_children.assert_called_once_with(gcs_folder, pattern=".*avro")
        mock_client.load_table_from_uri.assert_called_once_with(
            source_uris=["gs://test-bucket/test-path/file1.avro"],
